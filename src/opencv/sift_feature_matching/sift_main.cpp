...
Eigen::Matrix4d createHomogeneousMatrix(double R11, double R12, double R21,
                                        double R22, double T11, double T22) {
  Eigen::Matrix4d homogeneousMatrix;

  // Rotation matrix for rotation about z-axis
  double theta = std::atan2(R21, R11);
  Eigen::Matrix2d rotationMatrix;
  rotationMatrix << std::cos(theta), -std::sin(theta), std::sin(theta),
      std::cos(theta);

  // Fill in the homogeneous matrix
  homogeneousMatrix << rotationMatrix(0, 0), rotationMatrix(0, 1), 0, T11,
      rotationMatrix(1, 0), rotationMatrix(1, 1), 0, T22, 0, 0, 1, 0, 0, 0, 0,
      1;
...
int main() {
  ...
  cv::Mat original_1;
  cv::Mat original_2;
  cv::Mat image1 = cv::imread("src/opencv/sift_feature_matching/base_1_cp.png",
                              cv::IMREAD_GRAYSCALE);
  cv::Mat image2 = cv::imread("src/opencv/sift_feature_matching/base_2_cp.png",
                              cv::IMREAD_GRAYSCALE);
  ...
  Eigen::Matrix4d homogeneous = createHomogeneousMatrix(
      H.at<double>(0, 0), H.at<double>(0, 1), H.at<double>(1, 0),
      H.at<double>(1, 1), H.at<double>(0, 2), H.at<double>(1, 2));
  ...
  cv::imshow("Original original_2", original_1);
  cv::imshow("Original original_1", original_2);
