...
void GpuWork(const std::vector<int>& v) {
  thrust::device_vector<int> input(v.size());

  thrust::copy(v.begin(), v.end(), input.begin());

  for (const int n : {3, 5, 9}) {
    auto iter =
        thrust::find_if_not(thrust::device, input.begin(), input.end(),
                        [n] __device__(int x) -> bool { return x > n; });
    // Check if the element was found
    if (iter != input.end()) {
      std::cout << "input greater than " << n << '\n';
    } else {
      std::cout << "input does not greater than  " << n << '\n';
    }
